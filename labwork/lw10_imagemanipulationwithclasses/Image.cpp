...
Image::Image(const Image& object) : image(nullptr), width(0), height(0) {
...
Image& Image::operator=(const Image& object) {
  if (this != &object) {
    ...
    allocateImage(object.width, object.height);

    for (size_t i = 0; i < height; i++) {
      for (size_t j = 0; j < width; j++) {
        image[j][i] = object.image[j][i];
      }
    }
  }
  return *this;
